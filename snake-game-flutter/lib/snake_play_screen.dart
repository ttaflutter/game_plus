...
int foodPoints(FoodType t) {
  switch (t) {
    case FoodType.red:    return 1; // apple
    case FoodType.blue:   return 2; // blueberry
    case FoodType.purple: return 3; // grape
    case FoodType.yellow: return 5; // banana
  }
}

String foodAsset(FoodType t) {
  switch (t) {
    case FoodType.red:    return 'assets/img/apple.png';
    case FoodType.blue:   return 'assets/img/blueberry.png';
    case FoodType.purple: return 'assets/img/grape.png';
    case FoodType.yellow: return 'assets/img/banana.png';
  }
}
...
class _SnakePlayScreenState extends State<SnakePlayScreen> with SingleTickerProviderStateMixin {
  // ===== Board size: 20 x 15 =====
  ...
  List<List<int>> snake = [[7, 9], [7, 10], [7, 11]];
  ...
  void initState() {
    ...
    _foodCtrl = AnimationController(vsync: this, duration: const Duration(seconds: 2))
      ..repeat(reverse: true);
    _foodScale = Tween<double>(begin: 1.0, end: 1.2)
  ...
  void _onKey(RawKeyEvent e) {
    ...
    if (k == LogicalKeyboardKey.arrowUp || k == LogicalKeyboardKey.keyW) _setNextDirection('up');
    else if (k == LogicalKeyboardKey.arrowDown || k == LogicalKeyboardKey.keyS) _setNextDirection('down');
    else if (k == LogicalKeyboardKey.arrowLeft || k == LogicalKeyboardKey.keyA) _setNextDirection('left');
    else if (k == LogicalKeyboardKey.arrowRight || k == LogicalKeyboardKey.keyD) _setNextDirection('right');
    else if (k == LogicalKeyboardKey.space) { isPlaying ? _pauseGame() : startGame(); }
    else if (k == LogicalKeyboardKey.keyR) { startGame(); }
  }
  ...
  void _endGame() {
    ...
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        title: const Text('Game Over', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.w700)),
  ...
  void _createFoodSafe() {
    while (true) {
      ...
      if (!overlaps) { food = Food(x, y, _rollFoodType()); break; }
    }
  }

  // ===== Audio =====
  Future<void> _playStart() async { try { await _sfxStart.play(AssetSource('sfx/start.mp3')); } catch (_) {} }
  Future<void> _playEat()   async { try { await _sfxEat.play(AssetSource('sfx/eat.mp3'));   } catch (_) {} }
  Future<void> _playOver()  async { try { await _sfxOver.play(AssetSource('sfx/over.mp3'));  } catch (_) {} }
  ...
  String _bodyPath(List<int> prev, List<int> cur, List<int> next) {
    // thẳng
    if (prev[1]==cur[1] && next[1]==cur[1]) return '${_kSnakeDir}body_horizontal.png';
    if (prev[0]==cur[0] && next[0]==cur[0]) return '${_kSnakeDir}body_vertical.png';

    // khúc cua
    final hasUp    = (_eq(prev,[cur[0],cur[1]-1]) || _eq(next,[cur[0],cur[1]-1]));
    final hasDown  = (_eq(prev,[cur[0],cur[1]+1]) || _eq(next,[cur[0],cur[1]+1]));
    final hasLeft  = (_eq(prev,[cur[0]-1,cur[1]]) || _eq(next,[cur[0]-1,cur[1]]));
    final hasRight = (_eq(prev,[cur[0]+1,cur[1]]) || _eq(next,[cur[0]+1,cur[1]]));
                        ...
                        child: Stack(
                          children: [
                            ...
                            CustomPaint(
                              ...
                              painter: _GridPainter(rows: rows, cols: cols, cell: cell),
                            ),
...
class _GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int c = 1; c < cols; c++) {
      final x = c * cell; canvas.drawLine(Offset(x, 0), Offset(x, size.height), grid);
    }
    for (int r = 1; r < rows; r++) {
      final y = r * cell; canvas.drawLine(Offset(0, y), Offset(size.width, y), grid);
    }
  }
  @override
